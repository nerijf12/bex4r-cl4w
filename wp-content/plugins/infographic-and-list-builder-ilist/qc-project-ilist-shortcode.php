		...
		while ( $list_query->have_posts() ) 
		{
			...
			if(file_exists(OCOPD_TPL_DIR1 . "/$template_code/template.php")){
				...
				if($capture == 'true'){
					...
					echo '<div class="ilist-capture-wrap"><button class="ilist-download-btn" title="Click to take screenshot" >Save as Image</button><img src="" id="ilist-created-element"/> <a href='.esc_url("https://www.quantumcloud.com/products/infographic-maker-ilist/").' target="_blank" style="color:indianred"> ( Pro ) </a> </div>';
...
<?php
	...
	{
		echo "<div><p>No directory items was found.</p></div>";
	}
	//End IF
...
function ilist_custom_sort_by_tpl_upvotes($a, $b){
	...
	return (@$a['sl_thumbs_up']==''?0:$a['sl_thumbs_up']) * 1 < (@$b['sl_thumbs_up']==''?0:$b['sl_thumbs_up']) * 1;
}
?>